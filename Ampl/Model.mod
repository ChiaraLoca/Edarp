...
param l{V};			# Change in load at location N
...
s.t. thirteen 	{k in K, i in V, j in V : i!=j}:	L[k,i] + l[j] - G[k, i] * (1- X[k, i,j])				<= 	L[k,j]; 
s.t. fourteen 	{k in K, i in V, j in V : i!=j}:	L[k,i] + l[j] + G[k, i] * (1- X[k, i,j])				<= 	L[k,j]; 
